...
the normal state — `baraza.web.ui` serves a page saying so rather than a bare 404, which
is what distinguishes "you are in a source tree" from "your install is broken".
...
without standing up a server. `mcp/server.py` is a thin registration shim. The split
means the half that can be wrong in an interesting way does not need the optional
dependency to be checked.

### The server binds loopback, and there is no parameter to change it

The tool has no login. That is correct for something reading one person's own calendar on
their own laptop, and correct *only* while the socket cannot be reached from elsewhere. A
host argument would be a parameter with exactly one safe value, which is a parameter
somebody eventually widens. A test asserts the absence, sweeping everything the module
publishes rather than a written-down list of names.

## Three layers of test, and what each is for
...
The third one is the point of a separate job: this package declares its dependencies and
installing only those is what turns that from a claim into a check. A development
checkout has more on the path and would never notice a stray import.
...
parameters there — both forms are a syntax error. The values are module constants or a
...
1. `cd ui && npm ci && npm run build` — the wheel carries the compiled UI.
