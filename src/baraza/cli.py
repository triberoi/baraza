...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        #
        # `ImportError` is in the list because of `baraza mcp` without the optional
        # extra, which is the single most likely error anyone meets on this CLI. It was
        # missing at first, so the careful install instruction arrived as the last line
        # of a traceback — found by running the command, not by a test.
...
def _import(args: argparse.Namespace) -> int:
    ...
    try:
        if args.luma:
            ...
            if args.key:
                # `--key` alongside a folder used to be accepted and thrown away, while
                # `--help` said it was "saved for next time". It was
                # not, so the next `--luma` run asked for it again — and the organizer
                # had put a secret in their shell history for nothing. The behavior is
                # what changed, not the help text: the key is honored on this path too,
                # on the same terms the help states — probed first, so a typo is
...
def _report(args: argparse.Namespace) -> int:
    ...
    print(f"turned up: {percent(attended / committed)}% of those who said yes" if committed else "turned up: —")
    print()
    ...
    for row in turnout:
        ...
        print(
            ...
            f"{row.attended:>3} attended  {row.new_attendees:>3} new  {row.no_shows:>3} no-show"
        )
    return 0
