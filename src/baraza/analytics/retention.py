...
from baraza.analytics.members import AttendanceRecord, EventRecord, effective_status, validate_calendar
...
def retention_grid(
    ...
    *,
    ...
    # Read through the evidence, as every other view does, or this grid contradicts the
    # roster about the same member.
    evidence = {event.event_id: event.attendance_evidence for event in events}

    attended_months: dict[str, set[_Month]] = {}
    for record in attendances:
        if effective_status(record.status, evidence[record.event_id]) == "attended" and record.event_id in settled:
