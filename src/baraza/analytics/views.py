...
class EventAttendance:
    ...
    :param attendance_evidence: what this event measured, so a screen can mark the rows
        whose counts came from registrations rather than from people seen.
    """
    ...
    def show_rate(self) -> float | None:
        """Share of those who said yes that turned up. ``None`` when nobody said yes, and
        ``None`` for a :data:`REGISTRATION_ONLY` event, whose committed guests all read as
        attended and would return 1.0 every time.

        A turnout of zero against a real roster still reports ``0.0``: that one is a
        collapse and must keep saying so. A caller summing these must leave a ``None``
        event out of both halves rather than count it as 100%.
        """
        if self.attendance_evidence == REGISTRATION_ONLY:
            return None
...
class TimelineEntry:
    ...
    ``status`` is read through the event's evidence, as every count is, or this screen
    contradicts the roster about the same member.
    """
...
def event_attendance(
    ...
    *,
    ...
    A :data:`REGISTRATION_ONLY` event reads its committed guests as attended, so it reports
    no no-shows and no show rate.
    """
    ...
    def status_of(record: AttendanceRecord) -> str:
        """Every count below reads through here, so the evidence moves all of them together."""
        return effective_status(record.status, evidence[record.event_id])
    ...
    for record in sorted((r for r in attendances if status_of(r) == "attended"), key=order):
    ...
    for event in sorted(started, key=lambda e: (e.starts_at, e.event_id)):
        ...
        attended = [r for r in records if status_of(r) == "attended"]
        key = (event.starts_at, event.event_id)
        new = sum(1 for r in attended if first_attendance.get(r.member_id) == key)
        no_shows = sum(1 for r in records if status_of(r) == "no_show")
        rows.append(
            EventAttendance(
...
@dataclass(frozen=True)
class EventCohort:
    """One event's room, and how much of it came back.

    The cohort is everyone who attended event N, regulars as well as first-timers.

    :param returned: how many attended any LATER settled event. ``None`` for the most
        recent one, which has had nothing to return to. Do not render it as ``0``: that
        reports a collapse every time an organizer imports their newest event.
    """
    ...
    def return_rate(self) -> float | None:
        """Share of the room that came back. ``None`` when there is no later event, or
        nobody in the room to ask about."""
        if self.returned is None or not self.attended:
            return None
        return self.returned / self.attended
...
def event_cohorts(
    ...
    *,
    ...
    """Per-event cohort return, in calendar order, for events that are over.

    Reads the same settled cut and the same evidence as :func:`event_attendance`, so the
    two reconcile row by row.

    The last settled event carries ``returned=None`` rather than being dropped: its size is
    worth seeing, and omitting it looks like an import that failed.
    """
    starts = validate_calendar(events, attendances, now=now)
    settled = sorted(
        (event for event in events if event.over_by <= now),
        key=lambda e: (e.starts_at, e.event_id),
    )
    evidence = {event.event_id: event.attendance_evidence for event in events}

    # The same tie-break `event_attendance` uses, so "later" means one thing on both
    # screens when two events share an instant.
    rank = {event.event_id: index for index, event in enumerate(settled)}
    attended_ranks: dict[str, set[int]] = {}
    for record in attendances:
        if record.event_id not in rank:
            continue
        if effective_status(record.status, evidence[record.event_id]) == "attended":
            attended_ranks.setdefault(record.member_id, set()).add(rank[record.event_id])

    rows = []
    for index, event in enumerate(settled):
        room = [member_id for member_id, ranks in attended_ranks.items() if index in ranks]
        last = index == len(settled) - 1
        rows.append(
            EventCohort(
                event_id=event.event_id,
                title=event.title,
                starts_at=starts[event.event_id],
                attended=len(room),
                returned=(
                    None
                    if last
                    else sum(1 for member_id in room if any(r > index for r in attended_ranks[member_id]))
                ),
                attendance_evidence=event.attendance_evidence,
            )
        )
...
def member_timeline(
    ...
    evidence = {event.event_id: event.attendance_evidence for event in events}
    theirs = {
        r.event_id: effective_status(r.status, evidence[r.event_id])
