...
def overview(store_path: Path | str, *, now: datetime | None = None) -> dict[str, Any]:
    ...
    ``show_rate`` covers the events that recorded attendance, which is ``show_rate_events``
    of ``events_held``. The rest declared registration-only and have no turnout to report.
    """
    moment = now or _now()
    ...
    # Measured events only: a registration-only event reads every committed guest as
    # attended, so folding it in adds a 100% to both halves.
    ...
    return {
        ...
        "show_rate": round(sum(row.attended for row in measured) / committed, 3) if committed else None,
        # The scope of `show_rate`: a caller cannot otherwise tell that the denominator
        # covered fewer events than `events_held`.
        "show_rate_events": len(measured),
        ...
        "events": [
            {
                ...
                # On a registration-only row `attended` counts registrations, not people seen.
...
def retention(store_path: Path | str, *, now: datetime | None = None) -> dict[str, Any]:
    ...
    ``in_progress_month`` is the third reason a cell is null: the current month has not
    finished being counted, so its cells are blank in a way that is neither "no event" nor
    "beyond the grid".

    **Each cohort's returns name their own month.** A bare list is misreadable three ways,
    each of which turns a healthy community into a collapsing one in a summary nobody
    double-checks:

    * it is offset from the **cohort's** first month while ``months`` is the grid's axis,
      so lining the two up shifts every later cohort;
