...
Thin by design. Every endpoint here opens the store, projects it onto the analytics
input pair, calls one function from:mod:`baraza.analytics`, and serializes. No
arithmetic lives in this module — if a number is wrong, it is wrong in analytics,
where the tests that would catch it already are.

**Nothing is cached and nothing is precomputed.** A laptop calendar is small and the
roll-ups are cheap, so every request recomputes from the store. That is what makes
moving a threshold feel instant instead of needing an invalidation story, and it is
why:mod:`baraza.store` holds facts and not conclusions.

**``now`` is supplied here**, at the edge, and injected as a callable so tests can
pin it. Analytics stays a function of its arguments; this is the one place in the
package that is allowed to know what time it is.

**A fresh connection per request.** SQLite connections object to being used from a
thread other than the one that made them, and the server is threaded. One short-lived
connection per request sidesteps that entirely.

What it costs, on a 50-event / 200-member store: ``0.56 ms`` to open and close a read
connection, against ``3.3 ms`` to open, read the whole calendar and close. A read opens
read-only and takes no write lock, so a writer can take one while a read is in flight.

**Field names are snake_case**, matching the Python that produces them. There is no
frozen wire contract here to honor — the only consumer is this package's own UI, and
a translation layer between two things we both own would be pure overhead.
"""
...
from baraza.analytics import ScoredMember, Thresholds, analyze_members
    ...
    *,
    ...
    def put_event(event_id: str, body: EventBody) -> dict[str, Any]:
        ...
        try:
            ...
            # Read back rather than echo. The body carries a title and dates and
            # nothing else, and since an earlier review a field the caller says nothing
            # about is LEFT ALONE rather than blanked — so an event that already has a
            # venue and tags keeps them, and echoing the request would report empties
    ...
    def retention() -> dict[str, Any]:
        """The cohort grid, with what the UI needs to explain each kind of gap — a
