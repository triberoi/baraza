...
# --- an event that recorded no attendance ------------------------------------------
def _declare_june_unmeasured(page: Page, served) -> None:  # noqa: ANN001
    """Drive the real control on Settings, rather than writing to the store behind it.

    The point of doing it through the UI is that the declaration has to travel: a select
    on one screen has to change what four other screens say, and the wiring between them
    is the part a unit test cannot see.
    """
    page.goto(f"{served.origin}/settings?token={served.url.split('token=')[1]}")
    expect(page.get_by_role("heading", name="Attendance")).to_be_visible()
    page.get_by_label("What June meetup recorded").select_option("registration_only")


def test_declaring_an_event_unmeasured_survives_a_reload(page: Page, served) -> None:  # noqa: ANN001
    """It is a property of the store, not of the tab. An organizer who declares this and
    comes back tomorrow to a control that has forgotten would have no reason to trust the
    numbers it moved."""
    _declare_june_unmeasured(page, served)
    page.reload()
    expect(page.get_by_label("What June meetup recorded")).to_have_value("registration_only")


def test_an_unmeasured_event_is_marked_and_left_out_of_the_turnout(page: Page, served) -> None:  # noqa: ANN001
    """The surfacing rule, in the browser.

    Three things have to be true at once on the overview, and any one of them missing
    leaves a screen where the figure reads as measured when it is not: the event carries a
    mark, its own turnout cell is blank rather than a perfect score, and the headline rate
    says how much of the calendar it actually covered.
    """
    _declare_june_unmeasured(page, served)
    page.goto(served.url)

    expect(page.get_by_text("Registration only").first).to_be_visible()

    turned_up = page.locator(".tile").filter(has_text="Turned up")
    expect(turned_up).to_contain_text("measured across 4 of 5 events")

    page.get_by_text("Turnout as a table").click()
    turnout = page.locator("table").filter(has=page.get_by_role("columnheader", name="Turned up"))
    row = turnout.get_by_role("row").filter(has_text="June meetup")
    row.scroll_into_view_if_needed()
    expect(row.get_by_role("cell").nth(-1)).to_have_text("—")
    # Nobody was measured, so nobody was absent: the no-show column has to go to zero at
    # the same time, or the row reports two contradictory things about one event.
    expect(row.get_by_role("cell").nth(-2)).to_have_text("0")


def test_a_roster_row_says_which_of_its_attendances_were_unmeasured(page: Page, served) -> None:  # noqa: ANN001
    """The row somebody opens when they distrust the headline. A member counted in from a
    registration must not read the same as one who walked through a door."""
    _declare_june_unmeasured(page, served)
    page.goto(f"{served.origin}/people?token={served.url.split('token=')[1]}")

    row = page.get_by_role("row").filter(has_text="Sam Ghost")
    expect(row).to_contain_text("1 not measured")


def test_the_cohort_table_leaves_the_newest_event_blank(page: Page, served) -> None:  # noqa: ANN001
    """Nothing has happened since for its room to come back to. Drawn as 0% it reports a
    collapse on the newest event every single time an organizer imports one."""
    page.goto(f"{served.origin}/returning?token={served.url.split('token=')[1]}")
    # A figure named by its figcaption, not a heading — the same shape as every other
    # chart on these screens.
    expect(page.get_by_role("figure", name="Did the room come back?")).to_be_visible()

    cohorts = page.locator("table").filter(has=page.get_by_role("columnheader", name="Came to a later event"))
    newest = cohorts.get_by_role("row").filter(has_text="June meetup")
    newest.scroll_into_view_if_needed()
    expect(newest.get_by_role("cell").nth(-1)).to_have_text("—")

    first = cohorts.get_by_role("row").filter(has_text="January meetup")
    expect(first.get_by_role("cell").nth(-1)).to_contain_text("(")
