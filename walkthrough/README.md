...
this. That is the point — it needs a **built** front end and a browser binary, and
neither is present in a plain checkout or in the job that installs only baraza's declared
dependencies.

It is not skipped into nothing, which would make it telemetry rather than a check: the
`walkthrough` CI job exists to run it, builds the UI first, and installs the browser.
