...
// request never got one. Both halves matter: `Problem` used to decide "your session
// ended" by pattern-matching the MESSAGE, so a store path containing the word
// "Sessions" was reported as an expired token.
type Load<T> =
  | { state: 'loading' }
  | { state: 'ready'; data: T }
  ...
  useEffect(() => {
    ...
    get<T>(path)
      ...
      .catch(
        (error: unknown) => {
          if (!live) return
          // A non-`ApiError` means the request never reached a server — the usual cause
          // being that the organizer closed the terminal running `baraza serve`. That is
          // the likeliest runtime failure of a local tool and it used to be the only one
          // with no sentence: `String(error)` put `TypeError: Failed to fetch` on screen
          //.
    ...
    // `nonce` and `deps` are the point of this effect and `path` is its subject;
    // there is no exhaustive-deps rule running here to disable (this package's
    // `lint` script is `tsc --noEmit` and there is no eslint config), so the
    // suppression that used to sit here suppressed nothing.
  }, [path, nonce, ...deps])
...
export function App({ authorized }: { authorized: boolean }) {
  ...
  //
  // It used to render at whatever address the organizer landed on — `/` — so the instant
  // the first import succeeded and `has_data` flipped, the app fell through to the normal
  // routed tree, `route.name` was still `overview`, and React unmounted the Import screen
  // mid-flow. It took the import report with it: the counts, the skipped files, and the
  // list of events still needing a name and a date before their guests could be used. The
  // organizer landed on an Overview reporting fewer events than their folder contained,
