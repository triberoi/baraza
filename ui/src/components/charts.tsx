...
import type { Cohort, EventTurnout, Retention } from '../api'
    ...
    <figure className="chart">
      ...
      {funnel && (
        <p className="figure__note">
          ...
          count feeds the turnout figures — nobody who was never given a place can be a no-show.
        </p>
      )}
    </figure>
  )
...
export function RetentionGrid({ retention }: { retention: Retention }) {
  ...
  const cellFor = (cohort: Cohort, offset: number) => {
    ...
    // A blank in the month still running is "not counted yet", not "no event". The
    // fallback below used to answer 'no-event' for every null, which was harmless
    // while those were the only nulls — and became a wrong story the moment the
    // in-progress month started producing them.
                  ...
                  if (cell.kind === 'in-progress') {
                    return (
                      <td
                        ...
                        title="This month is still running — not counted yet"
                      >
