...
export function People({ people }: { people: Person[] }) {
  ...
  // `aria-sort` belongs on the columnheader, not the button inside it — on the button
  // it is an unsupported attribute, which is exactly what the walkthrough's axe pass
  // flagged the day it was added (its first finding, and a pre-existing one).
          ...
          <tbody>
            {rows.map((person) => (
              <tr key={person.member_id}>
                ...
                <td className="tabular">{person.events_attended}</td>
