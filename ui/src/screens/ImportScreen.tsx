 ...
 *
 * It is the first thing an organizer sees before the tool knows anything, AND a screen
 * on the nav forever after. It used to be only the first: it rendered when the store was
 * empty and nowhere else, so the core loop — drop next month's export in the folder,
 * press refresh — became unreachable the moment the first import worked, and so did
 * naming the events Luma could not identify. The only way back was
...
export function ImportScreen({ status, onImported }: { status: Status; onImported: () => void }) {
  ...
  // Pending events come from the STORE (`/api/import/pending`), not from the last
  // import's response. The list used to render only from that response, so it existed
  // exactly once: a reload, a navigation, or arriving from the CLI — whose output says
  // "name them in the app" — found this screen with no sign that work was waiting.
          ...
          >
            <label className="field">
              ...
              <input
                ...
                placeholder={status.luma_key_configured ? 'Saved — leave blank to reuse' : 'secret-…'}
                spellCheck={false}
              />
            </label>
            <button
 ...
 * the folder is read again, which is why the copy tells the organizer to KEEP the
 * folder. It used to say "their guests are already read", which is true of the pass
 * that read them and false of the store — an organizer who believed it and deleted
 * their exports lost the guests.
 */
function Pending({ pending, onNamed }: { pending: PendingEvent[]; onNamed: () => void }) {
  return (
    <div className="pending">
      ...
      <p className="muted">
        Luma could not tell baraza about {pending.length === 1 ? 'this one' : 'these'} — a private
        event, usually. Fill these in, keep the exports where they are, and read the folder again:
        that is what attaches the guests.
      </p>
      {pending.map((event) => (
