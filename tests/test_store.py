...
def test_a_store_written_before_the_column_gains_it_and_reads_as_measured(tmp_path: Path) -> None:
    """The upgrade path for every file already on an organizer's disk.

    Dropping the column and the version stamp is what a v3 file IS, so this exercises the
    convergence rather than describing it. The events must come back measured — a file
    that predates the question cannot have answered it, and defaulting the other way would
    silently withhold the turnout figure from every event they already have."""
    path = tmp_path / "b.db"
    _loaded(path).close()
    with sqlite3.connect(path) as raw:
        # Rebuilt rather than dropping the column, which SQLite refuses here: removing the
        # last column re-parses the stored CREATE and leaves the comment above it dangling
        # before the closing paren. `legacy_alter_table` stops the rename from rewriting
        # the foreign key in `attendances` to follow the table being retired.
        raw.executescript(
            """
            ...
            INSERT INTO events SELECT event_id, title, starts_at, ends_at, venue, tags FROM events_v3;
    ...
    upgraded = connect(path, create=True)
    assert {e.attendance_evidence for e in upgraded.events()} == {"checked_in"}
    assert upgraded.events() == EVENTS, "nothing else moved"
    upgraded.close()
...
# --- the organizer's declaration ------------------------------------------------
def test_a_re_import_does_not_reset_what_the_organizer_declared(tmp_path: Path) -> None:
    """**The property the whole feature stands on.** No Luma export carries attendance
    evidence — it is the organizer saying something about their own event — so the normal
    loop of dropping a newer export in the folder and pressing refresh must leave it
    alone. If a re-import reset it, the declaration would survive exactly until the next
    refresh and the mode would be worse than not having it.

    The re-import here is the real one, with a CHANGED title, so it proves the row was
    genuinely written over rather than skipped."""
    store = _loaded(tmp_path / "b.db")
    store.set_attendance_evidence("e1", "registration_only")

    store.upsert_events([replace(EVENTS[0], title="January, renamed")])

    (january,) = [e for e in store.events() if e.event_id == "e1"]
    assert january.title == "January, renamed", "the upsert has to have actually run"
    assert january.attendance_evidence == "registration_only"
    store.close()


def test_the_declaration_survives_reopening_the_file(tmp_path: Path) -> None:
    """It is a property of the artifact, not of a session. Copy the file to another
    machine and the roster reads the same — the same promise the thresholds make."""
    store = _loaded(tmp_path / "b.db")
    store.set_attendance_evidence("e2", "registration_only")
    store.close()

    reopened = connect(tmp_path / "b.db")
    assert {e.event_id: e.attendance_evidence for e in reopened.events()} == {
        "e1": "checked_in",
        "e2": "registration_only",
    }
    reopened.close()


def test_an_event_arrives_measured_unless_someone_says_otherwise(tmp_path: Path) -> None:
    """The default is the claim that nothing changed. Every store written before the
    column existed, and every event nobody has spoken about, reads exactly as it did."""
    store = _loaded(tmp_path / "b.db")
    assert {e.attendance_evidence for e in store.events()} == {"checked_in"}
    store.close()


@pytest.mark.parametrize("value", ["", "yes", "checked-in", "REGISTRATION_ONLY"])
def test_an_unrecognized_declaration_is_refused_at_the_write(tmp_path: Path, value: str) -> None:
    """Refused here, not at the read. A store holding a value the analytics layer rejects
    is a file that opens and then fails on every screen, and the write is the last moment
    anything can say which event caused it."""
    store = _loaded(tmp_path / "b.db")
    with pytest.raises(ValueError, match="attendance_evidence"):
        store.set_attendance_evidence("e1", value)
    assert {e.attendance_evidence for e in store.events()} == {"checked_in"}
    store.close()


def test_declaring_something_about_an_event_that_is_not_there_is_refused(tmp_path: Path) -> None:
    """Silently succeeding would report a saved setting the store does not hold, and the
    screen would show it until the next reload."""
    store = _loaded(tmp_path / "b.db")
    with pytest.raises(ValueError, match="no event"):
        store.set_attendance_evidence("nope", "registration_only")
    store.close()
...
def test_the_calendar_projection_is_all_analytics_can_see(tmp_path: Path) -> None:
    ...
    ``attendance_evidence`` was added on the same terms: it decides whether a committed
    guest with no check-in is an absence or an unmeasured one, so it is an input to every
    rate downstream. Re-pinned here, not loosened."""
    store = _loaded(tmp_path / "b.db")
    ...
    assert [vars(e) for e in events] == [
        {
            "event_id": "e1",
            "title": "January",
            "starts_at": D(2026, 1, 10),
            "ends_at": D(2026, 1, 10, 2),
            "attendance_evidence": "checked_in",
        },
        {
            "event_id": "e2",
            "title": "February",
            "starts_at": D(2026, 2, 10),
            "ends_at": None,
            "attendance_evidence": "checked_in",
        },
    ]
