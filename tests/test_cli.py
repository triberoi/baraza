...
def test_the_report_names_the_scope_when_the_rate_skipped_events(
    store: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """A rate that quietly covers half the calendar is the number this mode exists to stop
    printing, and the CLI is a surface like any other. The scope goes on the same line as
    the figure — a reader piping this into a summary sees one line, not the page."""
    db = connect(store)
    db.set_attendance_evidence("e1", "registration_only")
    db.close()

    assert main(["report", "--store", str(store)]) == 0
    out = capsys.readouterr().out
    assert "measured across 1 of 2 events" in out
    assert "registration only" in out, "the row itself has to carry the mark"


def test_the_report_says_nothing_about_scope_when_it_covered_everything(
    store: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """The control. A qualifier printed on every ordinary run is noise, and noise is what
    trains somebody to stop reading the line it is attached to."""
    assert main(["report", "--store", str(store)]) == 0
    out = capsys.readouterr().out
    assert "measured across" not in out
    assert "registration only" not in out


def test_the_report_withholds_the_rate_when_no_event_measured_anything(
    store: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Nothing to average, so there is no figure — and the line says which of the two
    reasons it is, because "—" alone reads as "nobody came"."""
    db = connect(store)
    db.set_attendance_evidence("e1", "registration_only")
    db.set_attendance_evidence("e2", "registration_only")
    db.close()

    assert main(["report", "--store", str(store)]) == 0
    assert "no event recorded attendance" in capsys.readouterr().out
